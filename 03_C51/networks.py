...
class Q_Network(nn.Module):
    def __init__(self, state_size, action_size, v_min, v_max, N, hidden=[64, 64]):
        ...
        self.z = torch.linspace(v_min, v_max, N).view(1, 1, -1).to(DEVICE)
    ...
    def forward(self, state):
        ...
        log_prob = F.log_softmax(x, dim=2)  # (batch_size,action_size,N)
        Q_values = log_prob.exp() * self.z  # (batch_size,action_size,N)
        Q_values = torch.sum(input=Q_values, dim=2, keepdim=False)  # (batch_size,action_size)
        return log_prob, Q_values
...
class Visual_Q_Networks(nn.Module):
    ...
    def __init__(self, num_frame, num_action, v_min, v_max, N):
        ...
        self.z = torch.linspace(v_min, v_max, N).to(DEVICE)
